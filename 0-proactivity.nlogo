...
to setup
  ...
  ask patches [
    sprout-students 1 [
      ...
      set INIT-SUBSCRIPTIONS mk-subscriptions ;; select inital message sources - only email for now
  ...
  ask students [
    ;; give everyone an initial empty group of friends
...
to-report mk-subscriptions
  let subs "email"
  report subs
end

to-report mk-connections
  ;; nobody, but NetLogo doesn't like getting nobody in place of an agentset of size 0
  let network students with [ [who] of self < 0 ] ;; no given id is negative
  report network
end
