...
to setup
  ...
  ask patches [
    sprout-students 1 [
      ...
      set INIT-SOCIABILITY mk-float-param (random-normal mean-sociability stdev-sociability) 1
      ...
      set sociability INIT-SOCIABILITY
  ...
  mk-student-connections
  ask students [
    set INIT-CONNECTIONS (turtle-set [other-end] of my-links)
...
to go
  ...
  ;; new group project time ;; 1200 instead of max-iters so that the projects are per semester
  if network-restructures > 0 and ticks mod (1200 / (network-restructures + 1)) = 0
  [
    ask links [ set hidden? true ] ;; hide the old links to make new ones visible - note that the old connections still exit
    ask students [ set connections INIT-CONNECTIONS ] ;; resetting groups - assuming the projects are sequential
    let dcs-groups mk-subsets students [ 3 4 ]
    mk-subgroup-connections (list dcs-groups)
    ask students [ set connections (turtle-set [other-end] of my-links) ]
  ]
...
to student-share
  ...
  let soc [sociability] of self
  let thr mk-float-threshold
  let conn [connections] of self

  ask self [
    ifelse received? and not shared? and soc > thr
...
to-report mk-subsets [ agtset subsizes ]
  let ls (list) ;; list of subsets
  let subset nobody
  while [ count agtset > item 0 subsizes ]
  [
    set subset (n-of (one-of subsizes) agtset) ;; make groups of friends!
    set ls (insert-item (length ls) ls subset) ;; insert another student group
    ;; remove students who have found a group from the set of all students
    ask subset [ set agtset agtset with [ self != myself ] ]
  ]
  ifelse count agtset < item 0 subsizes ;;length (list last ls)
  [ ;; for each student in the undersized set move them to another random group in the list
    ;; right now it's being inserted between two groups not inside one of them
    ;; foreach (list agtset) [ stu -> ask stu [ set ls insert-item (pick-random-pos ls) ls stu ] ]
    if not any? agtset [
      foreach (list agtset) [ stu -> ask stu
        [
          let pos pick-random-pos ls
          let old-group item pos ls
          let new-group (turtle-set self old-group)
          set ls replace-item pos ls new-group
        ]
      ]
    ]
  ]
  [ set ls insert-item (length ls) ls agtset ]
  report ls
end

to-report pick-random-pos [ collection ]
  let rnd random (length collection)
  report rnd
end

to mk-connections [ stus ]
  foreach (list stus) [ stu -> ask stu [ create-links-with other stus ] ]
end
...
to mk-student-connections
  let sociability-subgroups mk-sociability-subgroups 5
  let friend-groups mk-friend-groups sociability-subgroups
  mk-subgroup-connections friend-groups
end

to-report mk-sociability-subgroups [ bins ]
  let interval 1 / bins
  let subgroups (list)
  while [ bins > 0 ]
  [
    let tmp-subgroup students with [ sociability > (bins - 1) * interval and sociability <= bins * interval ]
    set subgroups insert-item 0 subgroups tmp-subgroup
    set bins bins - 1
  ]
  report subgroups
end

to-report mk-friend-groups [ subgroups ]
  let counter length subgroups
  let friend-groups (list)
  while [ counter > 0 ]
  [
    let tmp-friendgroup mk-subsets item (counter - 1) subgroups (list counter (counter + 1))
    set friend-groups insert-item 0 friend-groups tmp-friendgroup
    set counter counter - 1
  ]
  report friend-groups
end

to mk-subgroup-connections [ subgroups ]
  foreach subgroups [ subgroup -> foreach subgroup [ sgrp -> mk-connections sgrp ] ]
end
...
to show-student-colour [ param ]
  let colour-param (ifelse-value
    ...
    param = "avgprosoc" [ (proactivity + sociability) / 2 ]
...
to mk-plot [ plottype param ]
  ...
  (ifelse
    ...
    plottype = "hist" and param = "sociability" [ histogram [INIT-SOCIABILITY] of students ]
    plottype = "dots" and param = "proactivity" [ ask students [ plotxy INIT-PROACTIVITY percent-received ] ]
    plottype = "dots" and param = "sociability" [ ask students [ plotxy INIT-SOCIABILITY percent-received ] ]
  )
end
@#$#@#$#@
GRAPHICS-WINDOW
...
"" ""
PENS
"default" 0.1 1 -16777216 true "" "mk-plot \"hist\" \"sociability\""
...
243
show sociability
ask students [ show-student-colour \"sociability\" ]
NIL
1
T
OBSERVER
...
"" ""
PENS
"default" 1.0 2 -16777216 true "" "mk-plot \"dots\" \"sociability\""
...
843
367
network-restructures is how many times the students are introduced to new students (i.e. through group projects)
11
0.0
1
...
243
show avg (pro + soc)
ask students [ show-student-colour \"avgprosoc\" ]
NIL
1
T
OBSERVER
...
<experiments>
  <experiment name="experimentProactivity" repetitions="1" runMetricsEveryStep="false">
    ...
    <enumeratedValueSet variable="mean-sociability">
      <value value="0.5"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="max-iters">
      <value value="1200"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="network-restructures">
      <value value="2"/>
    </enumeratedValueSet>
    ...
    <enumeratedValueSet variable="stdev-sociability">
      <value value="0.17"/>
    </enumeratedValueSet>
  </experiment>
  ...
  <experiment name="experimentSociability" repetitions="1" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>average-percent</metric>
    <enumeratedValueSet variable="random-seed">
    ...
    <steppedValueSet variable="mean-sociability" first="0" step="0.05" last="1"/>
    <enumeratedValueSet variable="max-iters">
      <value value="1200"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="network-restructures">
  ...
  <experiment name="experimentRest" repetitions="1" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>average-percent</metric>
    <enumeratedValueSet variable="random-seed">
    ...
    <steppedValueSet variable="network-restructures" first="0" step="1" last="10"/>
    <enumeratedValueSet variable="stdev-proactivity">
      <value value="0.17"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="mean-proactivity">
